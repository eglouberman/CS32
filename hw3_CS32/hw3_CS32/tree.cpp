...
int countIncludes(const double a1[], int n1, const double a2[], int n2)
{
    ...
    if (n2>n1) return 0;
    if (n2 ==0) return 1; // we found one match
...
void order(double a[], int n)
{
    ...
    split(a, n, a[n-1], firstNotGreater, firstLess); // grab small values
    if (firstLess == n) // this means that we found the smallest element
    {
        exchange(a[n-1],a[firstNotGreater]); // we swap the last element with the smallest
        order(a, n-1); // we call order again, with one less element.
    }
    else{
        exchange(a[n-1], a[firstLess]); // we make firstLess the last element.
        order(a, n);
    }
}
