...
//  Hw4_CS32
//
//  Created by Elon Glouberman on 3/6/18.
//  Copyright © 2018 Elon Glouberman. All rights reserved.
//
...
void insertion_sort(vector<Sensor>& s, bool comp(const Sensor&, const Sensor&))
{
    ...
    for (int i =1; i< s.size(); i++)
    {
        ...
        while (j>0)
        {
            if (compareSensor(s[j],s[k])) // if right hand side is less than left hand side
            {
                swap(s[j], s[k]);
                k--;
                j--;
            }
            else{ // we are in the right order
                j=0; // we break out of the while loop
            }
        }
    }
...
int main()
{
    ...
    // destruction time.
    {
        ...
        {
            vec.push_back(&auxSensors[i]); // each pointer pointing to auxSensor's indexes
        }
        
        sort(vec.begin(), vec.end(), compareSensorPtr); // sort it with regular sorting algorithm
        
        for (int i=0; i< vec.size(); i++) //put everyhting in sensors
            sensors[i] = *vec[i];
        
//        while (vec.size()>0)
//        {
//            delete vec[vec.size()-1]; // delete vec
//            vec.pop_back();
//        }
